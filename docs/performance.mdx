...
All processing currently happens client-side. For scenarios where you want to avoid any client-side pixel scanning (static sites, SSR, build-time optimization), we're exploring a Node.js adapter that would pre-compute `MeasurementResult` data using `@napi-rs/canvas`, letting you ship pre-measured data and skip detection entirely at runtime.

The benchmark suite already proves this works -- it runs the same `measureWithContentDetection` code server-side using `@napi-rs/canvas` as the canvas backend. Follow the [GitHub repo](https://github.com/sanity-labs/logo-soup) for updates.
